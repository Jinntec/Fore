...
export class XfForm extends PolymerElement {
    ...
    _itemAppended(e){
        ...
        // todo - this doesn't seem to work - proxies keep on hanging around?
        this.proxies = [];
...
*/
    ...
    _modelReady() {
        ...
        this.refresh();
        console.log('### _modelReady uiStates: ', this.modelItems);
    }
    ...
    resolveProxy(bindId, boundElement) {
        ...
        if (boundElement.hasOwnProperty('proxy')) {
            // console.log('resolve - already exists on element. Returning it: ', boundElement.proxy);
            ...
            if (this.proxies[bindId] === undefined) {
                ...
                // console.log('>>>>> resolve new proxy ', p);
                return p;
            } else {
...
/*
                    if (window.BOUND_ELEMENTS.indexOf(value.nodeName.toUpperCase()) != -1) {
                        ...
                        // possibly a hack...
                        // if(boundElements.indexOf(value) == -1){
                            boundElements.push(value);
                        // }
                    }
*/
                    if(Array.isArray(value)){
                        target.boundElements = [];
                    }else if (window.BOUND_ELEMENTS.indexOf(value.nodeName.toUpperCase()) != -1) {
                        // console.log('added bound control: ', value);
                        ...
                        // possibly a hack...
                        if(target.boundElements.indexOf(value) == -1){
                ...
                if(key === 'append'){
                    console.log('##### proxy append ', value);
                    // target.bind.push([{'id':'b-task','value':'bar'}]);
                    target.bind.push(value);
                }
...
*/
        ...
        console.log('bound Elements on bind? ', bind);
    ...
    _addUIState(bindid, uiState){
        const entry = this.uiStates[bindId];
        if (entry === undefined) {
            // this.proxies['items'] = new Array();
            this.uiStates[bindId] = [];
            this.uiStates[bindId][0] = proxy;
        } else {
            this.uiStates[bindId].push(proxy);
        }
        console.log('_addUIState uiStates updated', this.uiStates);
    }

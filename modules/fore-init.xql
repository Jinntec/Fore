...
module namespace init="http://existsolutions.com/fore/compile";

import module namespace xqgen="http://www.tei-c.org/tei-simple/xquery/xqgen";

declare function init:init-instances($model as element(xf-model)) {
    <map>
    {
...
(:~
 ...
 :)
declare function init:process-nested-binds($bind as element(xf-bind), $func as function(*)) {
...
(:~
 ...
 :)
declare function init:validate($bind as element(xf-bind)) {
    let $log := util:log('info', 'init:validate bind ' || $bind/@id)
    return
    <item>
        <let var="invalid">
            <expr>
...
declare function init:output($bind as element(xf-bind)) {
    <code>runtime:output({($bind/@ref/string(), $bind/@set/string())}, {($bind/@relevant/string(), "true()")[1]}, {serialize($bind)},1,"")</code>
};
...
(:~
 ...
 :)
declare function init:get-change-set($bind as element(xf-bind)) {
    if ($bind/@calculate) then
        ...
        <item><sequence/></item>,
        (: Recursively check sub-bindings :)
        init:process-nested-binds($bind, init:get-change-set#1)
};


declare function init:main($model as element(xf-model),$instances as element(*), $debug as xs:boolean?) {
    let $code :=
        <xquery>

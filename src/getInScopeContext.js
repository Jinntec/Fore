...
export default function getInScopeContext(node, ref) {
    ...
    if (
        parentBind &&
        (parentBind.nodeName === 'FX-GROUP' ||
            parentBind.nodeName === 'FX-CONTROL')
    ) {
        return parentBind.nodeset;
    }
    ...
    if (
        parentActionWithIterateExpr &&
        parentActionWithIterateExpr.currentContext
    ) {
    ...
    if (repeatItemFromAttrs) {
        ...
        const repeatFromAttributes = XPathUtil.getClosest(

...
export class FxModel extends HTMLElement {
  ...
  setInScopeVariables(inScopeVariables) {
    ...
    this.data.forEach((dataElement, id) => {
      let typedDataFactory = null;
      switch (dataElement.type) {
        case 'xml':
          typedDataFactory = createTypedValueFactory('element()?');
          break;
        case 'json':
          // TODO: array or map?
          typedDataFactory = createTypedValueFactory('map(*)?');
          break;
        case 'text':
          typedDataFactory = createTypedValueFactory('string()?');
          break;
      }

      inScopeVariables.set(id, {
        // TODO: bad check in xpath-evaluation
        nodeType: -1,
        get value() {
          return typedDataFactory(dataElement.data, domFacade);
        },
      });
    });
  }

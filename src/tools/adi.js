...
class ADI {
    constructor(rootElement, instance) {
        ...
        this.drawDOM(
            this.document,
            this.domView.querySelector('.adi-tree-view'),
            true,
        );
        document.addEventListener(
            'execute-action',
            (e) => this.processExecuteAction,
        );
    }
...
*/
    ...
    loadOptions() {
        ...
        userOptions =
            JSON.parse(window.localStorage.getItem('ADI.options')) || {};
    ...
    newTreeNode(sourceNode) {
        ...
        if (sourceNode.nodeName.startsWith('FX-')) {
            ...
            adiNode += Fore.isActionElement(sourceNode.nodeName)
                ? ' action'
                : '';
        }
...
*/
                    const attrString = Array.from(sourceNode.attributes)
                        .filter((attr) =>
                            this.isInstanceViewer
                                ? true
                                : isAttributeShown(attr.name, sourceNode),
                        )
    ...
    drawDOM(root, elem, isRoot) {
        if (typeof root !== 'object') {
            throw new Error(
                `drawDOM: Expected argument root of type object, ${typeof root} given.`,
            );
        }
        ...
        if (isRoot && this.options.nodeTypes.indexOf(root.nodeType) !== -1) {
            ...
            if (this.hasRequiredNodes(root)) {
                newNode.appendChild(
                    newElement('ul', { 'data-open': true, class: adiNode }),
                );
...
*/
                        ...
                        addClass(
                            newNode.querySelector('.adi-trigger'),
                    ...
                    if (this.getSelected() === node) {
                        const span = newNode.querySelector(
                            'span.adi-normal-node',
                        );
    ...
    refreshUI(refreshOpts) {
        ...
        if (refreshOpts) {
            this.optsView.querySelector('[data-opt="transparent"]').checked =
                this.options.transparent;
            this.optsView.querySelector('[data-opt="saving"]').checked =
                this.options.saving;
            this.optsView.querySelector('[data-opt="omitEmptyText"]').checked =
                this.options.omitEmptyText;
            this.optsView.querySelector('[data-opt="makeVisible"]').checked =
                this.options.makeVisible;
            this.optsView.querySelector('[data-opt="foldText"]').checked =
                this.options.foldText;
        ...
        this.domView.querySelector('.adi-content').style.height =
            `${this.domView.clientHeight}px`;
        if (!this.isInstanceViewer) {
...
*/
    ...
    changeOption(e) {
        ...
        if (data.indexOf('nodeTypes') !== -1) {
            ...
            if (val) {
                ...
                this.options.nodeTypes.splice(
                    this.options.nodeTypes.indexOf(type),
                    1,
                );
            }
        } else {
    ...
    horizontalResize(e) {
        ...
        if (
            nSplit >= this.options.minSplit &&
            nSplit <= 100 - this.options.minSplit
        ) {
    ...
    handleActive(e) {
        ...
        if (
            target &&
            target.classList &&
            target.classList.contains('adi-end-node')
        ) {
...
*/
        ...
        if (
            selected &&
            typeof selected.getModelItem === 'function' &&
            selected.getModelItem()?.node
        ) {
            ...
            window.document.dispatchEvent(
                new CustomEvent('log-active-element', {
                    detail: { target: selectedElement },
                }),
            );
        }
    ...
    highlightElement(event) {
        ...
        if (sourceNode.classList.contains('adi-end-node')) {
            sourceNode =
                sourceNode.parentNode.querySelector('.adi-normal-node');
        }

        const inspectorNode =
            this.sourceNodeByInspectorNodeLookup.get(sourceNode);
        ...
        if (inspectorNode) {
            if (event.type === 'mouseover') {
                ...
                inspectorNode.setAttribute(
                    'style',
                    `outline: 2px solid blue; ${this.styleBackup}`,
                );
    ...
    handleLookup(e) {
        ...
        if (e.type === 'mouseover') {
            ...
            target.setAttribute(
                'style',
                `outline: 1px dashed red; ${this.styleBackup}`,
            );
            return;
        }
        if (e.type === 'mouseout') {
        ...
        this.elemLookup = false;
        removeClass(
            this.menuView.querySelector('.adi-menu-lookup'),
            'adi-active',
        );
        target.setAttribute('style', this.styleBackup);
    ...
    removeEvent(elem, evt, fn, wasDelegated) {
        if (typeof elem !== 'object') {
            throw new Error(
                `addEvent: Expected argument elem of type object, ${typeof elem} given.`,
            );
        }
        ...
        if (wasDelegated) {
            for (
                let i = 0, len = this.delegatedEvents.length;
                i < len;
                i += 1
            ) {
        ...
        */
        ...
        const redrawUi = () => {
            if (this.instanceId !== '#document') {
                const instance = window.document.querySelector(
                    `#${this.instanceId}`,
                );
                this.document = instance.getInstanceData();
            }
        ...
        document.addEventListener(
            'mousemove',
            (event) => this.verticalResize(event),
            false,
        );
        document.addEventListener(
            'mousemove',
            (event) => this.horizontalResize(event),
            false,
        );
        // window resize
        window.addEventListener(
            'resize',
            (event) => this.refreshUI(event),
            false,
        );
        ...
        this.addEventDelegate(
            ...
            (e) => {
                const target = e ? e.target : window.event.srcElement;
                addClass(
                    target.parentNode.querySelector('.adi-normal-node'),
                    'hover',
                );
            },
            false,
        ...
        document.addEventListener('handle-active', (e) => {
            ...
            if (this.options.makeVisible) {
                const wrap = this.domView.querySelector('.adi-content');
                if (
                    target.offsetTop >= wrap.clientHeight ||
                    target.offsetTop <= wrap.scrollTop
                ) {
                    wrap.scrollTop =
                        target.offsetTop - Math.floor(wrap.clientHeight / 2);
                }
            }
        ...
        document.addEventListener(
            'execute-action',
            (e) => this.processExecuteAction(event),
            {
                capture: true,
            },
        );

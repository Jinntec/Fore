class FxMinimap extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.find = (sel) =>
            Array.from((this.viewport || document).querySelectorAll(sel));
    ...
    apply_styles(styles) {
        Object.keys(styles).forEach((sel) => {
            ...
            this.find(sel).forEach((el) => {
                this.draw_rect(
                    this.rect_rel_to(this.rect_of_el(el), this.root_rect),
                    col,
                );
            });
        });
    ...
    draw() {
        this.root_rect = this.viewport
            ? this.rect_of_content(this.viewport)
            : this.rect_of_doc();
        this.view_rect = this.viewport
            ? this.rect_of_viewport(this.viewport)
            : this.rect_of_win();
        this.scale = this.calc_scale(this.root_rect.w, this.root_rect.h);

        this.resize_canvas(
            this.root_rect.w * this.scale,
            this.root_rect.h * this.scale,
        );
        ...
        this.draw_rect(
            this.rect_rel_to(this.root_rect, this.root_rect),
            this.settings.back,
        );
    ...
    on_drag(ev) {
        ...
        const x =
            (ev.pageX - cr.x) / this.scale - this.view_rect.w * this.drag_rx;
        const y =
            (ev.pageY - cr.y) / this.scale - this.view_rect.h * this.drag_ry;

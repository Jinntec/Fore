...
class FxJsonInstance extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.foreSelector = this.hasAttribute('fore')
            ? this.getAttribute('fore')
            : 'fx-fore'; // default to first one in doc
        this.render();
    }
    ...
    render() {
      ...
      `;
        ...
        const instanceId = this.hasAttribute('instance')
            ? this.getAttribute('instance')
            : 'default';
        const fore = document.querySelector(this.foreSelector);
        if (!fore) {
            throw new Error(
                `this '${this.foreSelector}' does not match a fx-fore element`,
            );
        }
      ...
      `;
        ...
        if (this.isCollapsable(json))
            tree = '<a href=\'#\' class="json-toggle"></a>'.concat(tree); // Insert HTML in target DOM element
        ...
        toggles.forEach((toggle) => {
            toggle.addEventListener(
                'click',
                this._handleToggleEvent.bind(this),
            );
        });
    ...
    _handleToggleEvent(event) {
        ...
        const subTarget = this._siblings(
            elm,
            'ul.json-dict, ol.json-array',
            (el) => {
                el.style.display =
        ...
        for (let i = 0; i < subTarget.length; i += 1) {
            if (!this._isHidden(subTarget[i])) {
                ...
                this._siblings(subTarget[i], '.json-placeholder', (el) =>
                ...
                subTarget[i].insertAdjacentHTML(
                    'afterend',
                    '<a href class="json-placeholder">'.concat(
                        placeholder,
                        '</a>',
                    ),
                );
        ...
        if (typeof json === 'string') {
            ...
            if (json.length > 0) {
                ...
                for (let i = 0; i < json.length; i += 1) {
                    html += '<li data-key-type="array" data-key="'.concat(
                        i,
                        '">',
            ...
            if (keyCount > 0) {
                ...
                for (const key in json) {
                    if (json.hasOwnProperty(key)) {
                        html += '<li data-key-type="object" data-key="'.concat(
                            key,
                            '">',
                        );
                        const keyRepr = options.outputWithQuotes
                            ? '<span class="json-string">"'.concat(
                                  key,
                                  '"</span>',
                              )
                        ...
                        if (this.isCollapsable(json[key])) {
                            html += '<a href=\'#\' class="json-toggle">'.concat(
                                keyRepr,
                                '</a>',
                            );
                        } else {

...
export class Relevance {
    ...
    static selectRelevant(element, type) {
        ...
        switch (type) {
            ...
            default:
                console.warn(
                    `relevance selection not supported for type:${element.type}`,
                );
    ...
    static _relevantXmlNodes(element) {
        ...
        if (
            element.nodeset.children.length === 0 &&
            Relevance._isRelevant(element, element.nodeset)
        ) {
            return element.nodeset;
        }
            ...
            if (Relevance._isRelevant(element, n)) {
                ...
                if (attributes) {
                    Array.from(attributes).forEach((attr) => {
                        if (
                            element.nonrelevant === 'empty' &&
                            !Relevance._isRelevant(element, attr)
                        ) {

...
function getCachedNamespaceResolver(xpath, node) {
    ...
    return (
        createdNamespaceResolversByXPathQueryAndNode.get(xpath).get(node) ||
        null
    );
}
...
function setCachedNamespaceResolver(xpath, node, resolver) {
    if (!createdNamespaceResolversByXPathQueryAndNode.has(xpath)) {
        return createdNamespaceResolversByXPathQueryAndNode.set(
            xpath,
            new Map(),
        );
    }
    return createdNamespaceResolversByXPathQueryAndNode
        .get(xpath)
        .set(node, resolver);
}
...
export function resolveId(id, sourceObject, nodeName = null) {
    ...
    */
    ...
    const ownerForm =
        sourceObject.localName === 'fx-fore'
            ? sourceObject
            : sourceObject.closest('fx-fore');
...
function createNamespaceResolver(xpathQuery, formElement) {
    ...
    if (instanceReferences.length === 1) {
        ...
        if (instanceReferences[0] === 'default') {
            ...
            instance =
                actualForeElement &&
                actualForeElement.querySelector('fx-instance');
        } else {
            instance = resolveId(
        ...
        if (instance && instance.hasAttribute('xpath-default-namespace')) {
            const xpathDefaultNamespace = instance.getAttribute(
...
function createNamespaceResolverForNode(query, contextNode, formElement) {
    if (
        ((contextNode && contextNode.ownerDocument) || contextNode) ===
        window.document
    ) {
...
function functionNameResolver({ prefix, localName }, _arity) {
    switch (localName) {
        ...
        default:
            if (
                prefix === '' &&
                globallyDeclaredFunctionLocalNames.includes(localName)
            ) {
...
function getVariablesInScope(formElement) {
    ...
    if (closestActualFormElement.inScopeVariables) {
        for (const key of closestActualFormElement.inScopeVariables.keys()) {
            const varElementOrValue =
                closestActualFormElement.inScopeVariables.get(key);
...
export function evaluateXPathToNodes(xpath, contextNode, formElement) {
    try {
        ...
        const result = fxEvaluateXPathToNodes(
            xpath,
            contextNode,
            null,
            variablesInScope,
    ...
    (dynamicContext, string) => {
        ...
        if (instance) {
            if (instance.getAttribute('type') === 'json') {
                ...
                const def = new XMLSerializer().serializeToString(
    ...
    const convert = (obj, parent) => {
        ...
        if (type === 'number') {
            ...
            Object.entries(obj).forEach(([key, value]) => {
                if (value) {
                    const childNode = document.createElement(

...
*/
            ...
            if (children) {
                for (const element of Array.from(children)) {
                    ...
                    if (
                        Fore.isUiElement(element.nodeName) &&
                        typeof element.refresh === 'function'
                    ) {
                        /**
...
*/
        ...
        if (children) {
            Array.from(children).forEach((node) => {
                ...
                if (
                    ...
                    const conv =
                        targetElement.ownerDocument.createElement(lookFor);
                    ...
                    if (node.nodeType === Node.TEXT_NODE) {
                        const copied =
                            targetElement.ownerDocument.createTextNode(
                                node.textContent,
                            );
                    ...
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        const copied =
                            targetElement.ownerDocument.createElement(
                                node.nodeName,
                            );
                        targetElement.appendChild(copied);
    ...
    static async handleResponse(response) {
        ...
        if (status >= 400) {
            ...
            alert(
                `response status:  ${status} - failed to load data for '${this.src}' - stopping.`,
            );

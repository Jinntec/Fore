...
export default class FxControl extends XfAbstractControl {
    ...
    _getValueOfWidget() {
        if (this.valueProp === 'selectedOptions') {
            ...
            return [...this.widget.selectedOptions]
    ...
    connectedCallback() {
        ...
        this.addEventListener('mousedown', (e) => {
            ...
            if (
                !Fore.isWidget(e.target) &&
                !e.target?.classList.contains('fx-hint')
            ) {
                e.preventDefault();
        ...
        const defaultValueProp = this.widget.hasAttribute('multiple')
            ? 'selectedOptions'
            : 'value';
    ...
    _replaceNode(node) {
        ...
        if (node.nodeType === Node.ATTRIBUTE_NODE) {
            ...
            Fore.dispatch(this, 'warn', {
                message:
                    'trying to replace a node that is neither an Attribute, Elemment or Text node',
            });
        }
    ...
    async updateWidgetValue() {
        ...
        if (this.hasAttribute('as')) {
            ...
            if (as === 'text') {
                ...
                const pretty = Fore.prettifyXml(
                    serializer.serializeToString(this.nodeset),
                );
                widget.value = pretty;
            }
        ...
        if (this.src && !this.loaded && this.modelItem.relevant) {
            ...
            if (this.initial) {
                this.initialNode = evaluateXPathToFirstNode(
                    this.initial,
                    this.nodeset,
                    this,
                );
            ...
            imported.addEventListener(
                ...
                (e) => {
                    ...
                    if (this.initial) {
                        ...
                        doc.firstElementChild.appendChild(
                            this.initialNode.cloneNode(true),
                        );

...
export class FxRepeat extends withDraggability(ForeElementMixin, false) {
  ...
  async refresh(force) {
    ...
    DependencyTracker.getInstance().registerControl(this.ref, new ControlBinding(this));
    ...
    for (let i = 0; i < Math.abs(diff); i++) {
      if (diff < 0) {
        ...
        const itemToRemove = repeatItems[deletedIndexes[i] - 1];
        if (itemToRemove) {
          itemToRemove.remove();
        }
      }
    ...
    const minLength = Math.min(deletedIndexes.length, insertedIndexes.length);
    for (let i = 0; i < minLength; i++) {
      const oldItem = repeatItems[deletedIndexes[i] - 1];
      if (oldItem) {
        oldItem.index = insertedIndexes[i];
        oldItem.nodeset = this.nodeset[insertedIndexes[i] - 1];
      }
    }

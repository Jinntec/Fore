...
class FxLoad extends AbstractAction {
    ...
    connectedCallback() {
        ...
        this.attachTo = this.hasAttribute('attach-to')
            ? this.getAttribute('attach-to')
            : '_self';

        // Add a 'doneEvent' to block the action chain untill the event fired on the element we're
        // loading something into.
        this.awaitEvent = this.hasAttribute('await')
            ? this.getAttribute('await')
            : '';
    ...
    async perform() {
            ...
            this.dispatchEvent(
                new CustomEvent('error', {
                    ...
                    detail: {
                        origin: this,
                        message:
                            'neither template element nor Url was specified.',
                        level: 'Error',
                    },
                }),
            );
        ...
        const resolvedUrl = this.evaluateAttributeTemplateExpression(
            this.url,
            this,
        );
    ...
    _attachToElement(content) {
        ...
        if (content.nodeType) {
            ...
            try {
                effectiveContent = new DOMParser().parseFromString(
                    content,
                    'text/html',
                ).firstElementChild;
            } catch (e) {
        ...
        if (this.attachTo.startsWith('#')) {
            ...
            if (effectiveContent.querySelector('fx-fore')) {
                resolved.append(
                    effectiveContent.querySelector('fx-fore').cloneNode(true),
                );
                return;
            }

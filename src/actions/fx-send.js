...
class FxSend extends AbstractAction {
    ...
    connectedCallback() {
        ...
        this.target = this.hasAttribute('target')
            ...
            : null;
        this.connection = this.hasAttribute('connection')
            ? this.getAttribute('connection')
            : null;
    }
    ...
    async perform() {
        ...
        if (this.connection) {
            const connectionElement = this.getModel().querySelector(
                `#${this.connection}`,
            );
        ...
        if (this.url) {
            const resolved = this.evaluateAttributeTemplateExpression(
                this.url,
                this,
            );

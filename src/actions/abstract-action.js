...
export class AbstractAction extends ForeElementMixin {
    ...
    connectedCallback() {
        ...
        this.propagate = this.hasAttribute('propagate')
            ? this.getAttribute('propagate')
            : 'continue';
        ...
        */
        ...
        this.whileExpr = this.hasAttribute('while')
            ? this.getAttribute('while')
            : null;
        this.delay = this.hasAttribute('delay')
            ? Number(this.getAttribute('delay'))
            : 0;
        this.iterateExpr = this.hasAttribute('iterate')
            ? this.getAttribute('iterate')
            : null;
    ...
    async execute(e) {
        ...
        if (
            ...
            */
            if (
                e.target.closest('fx-fore') &&
                e.target.closest('fx-fore') !== this.closest('fx-fore')
            ) {
        ...
        if (
            this.ifExpr &&
            !evaluateXPathToBoolean(this.ifExpr, getInScopeContext(this), this)
        ) {
            this._finalizePerform(resolveThisEvent);
            return;
        }
    ...
    async handleIterateExpr() {
        try {
            ...
            const contextSequence = evaluateXPath(
                this.iterateExpr,
                getInScopeContext(this),
                this,
            );
    ...
    actionPerformed(force = false) {
        ...
        if (
            ...
            !XPathUtil.contains(
                FxFore.outermostHandler.ownerDocument,
                FxFore.outermostHandler,
            )
        ) {

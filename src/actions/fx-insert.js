...
export class FxInsert extends AbstractAction {
    ...
    `;
        ...
        this.position = this.hasAttribute('position')
            ...
            : 'after';
        this.origin = this.hasAttribute('origin')
            ? this.getAttribute('origin')
            : null; // last item of context seq
    ...
    _cloneOriginSequence(inscope, targetSequence) {
         ...
         */
                if (
                    ...
                    this.getOwnerForm().createNodes
                ) {
                    const repeat = this.getOwnerForm().querySelector(
                        this.origin,
                    );
                    originSequenceClone = repeat.createdNodeset.cloneNode(true);
                    if (!originSequenceClone) {
                        console.error(
                            `createdNodeset for repeat ${this.origin} does not exist`,
                        );
                    }
                } else {
                    // originTarget = evaluateXPathToFirstNode(this.origin, inscope, this);
                    originTarget = evaluateXPathToFirstNode(
                        this.origin,
                        inscope,
                        this,
                    );
                    if (
                        Array.isArray(originTarget) &&
                        originTarget.length === 0
                    ) {
                        console.warn(
                            'invalid origin for this insert action - ignoring...',
                            this,
                        );
    ...
    async perform() {
        ...
        if (this.hasAttribute('ref')) {
            if (inscope) {
                ...
                inscope = getInScopeContext(
                    this.getAttributeNode('ref'),
                    this.ref,
                );
                targetSequence = evaluateXPathToNodes(this.ref, inscope, this);
            }
        ...
        const originSequenceClone = this._cloneOriginSequence(
            inscope,
            targetSequence,
        );

...
*/
    ...
    if (resolvedUrl.startsWith('#echo')) {
      const parsedData = this._parse(serialized, data);
      this._handleResponse(parsedData);
  ...
  _getTargetData() {
    ...
    if (this.data) {
      ...
      targetData = this.model.getDefaultData();
    }
    if (!targetData) {
    ...
    if (this.replace === 'data') {
      if (targetData) {
        if (this.targetref) {
          ...
          const [theTarget] = evaluateXPath(this.into, targetData.data.firstElementChild, this);
          console.log('theTarget', theTarget);

...
export class FxSubmission extends ForeElementMixin {
    ...
    connectedCallback() {
        ...
        this.instance = this.hasAttribute('instance')
            ? this.getAttribute('instance')
            : null;

        /** if present will determine XPath where to insert a response into when mode is 'replace' */
        this.into = this.hasAttribute('into')
            ? this.getAttribute('into')
            : null;

        /** http method */
        this.method = this.hasAttribute('method')
        ...
        this.targetref = this.hasAttribute('targetref')
            ? this.getAttribute('targetref')
            : null;
...
*/
        ...
        if (resolvedUrl.startsWith('localStore:')) {
            ...
            if (this.method === 'delete') {
                const key = resolvedUrl.substring(resolvedUrl.indexOf(':') + 1);
                localStorage.removeItem(key);
                const newInst = new DOMParser().parseFromString(
                    '<data></data>',
                    'application/xml',
                );
                this.replace = 'instance';
        ...
        try {
            ...
            if (!response.ok || response.status > 400) {
                ...
                Fore.dispatch(this, 'submit-error', {
                    message: `Error while submitting ${this.id}`,
                });
                return;
            }
...
*/
    ...
    _getUrlExpr() {
        return this.storedTemplateExpressions.find(
            (stored) => stored.node.nodeName === 'url',
        );
    }
            ...
            if (targetInstance) {
                if (this.targetref) {
                    ...
                    if (this.responseMediatype === 'application/json') {
                        console.warn(
                            'targetref is not supported for application/json responses',
                        );
                    }
                } else if (this.into) {
                    const [theTarget] = evaluateXPath(

...
import {
    evaluateXPath,
    globallyDeclaredFunctionLocalNames,
...
export default function registerFunction(functionObject, formElement) {
    ...
    if (!signatureParseResult) {
        throw new Error(
            `Function signature ${functionObject.signature} could not be parsed`,
        );
    }

    const { prefix, localName, params, returnType } =
    ...
    const paramParts = params
        ? params.split(',').map((param) => {
              const match = param.match(
                  /(?<variableName>\$[^\s]+)(?:\sas\s(?<varType>[^\s]+))/,
              );
              if (!match) {
    ...
    switch (type) {
        case 'text/javascript': {
            ...
            registerCustomXPathFunction(
                ...
                returnType || 'item()*',
                (...args) =>
                    fun.apply(formElement.getInScopeContext(), [
                        ...args,

...
describe('instance Tests', () => {
  ...
  it('evaluates xpath in its default context', async () => {
    ...
    const result = data.evalXPath('//foobar');
  ...
  it('provides default evaluation context', async () => {
    ...
    const data = await new DataElement(el).init();
    const context = data.getDefaultContext();
  ...
  it('resolves data with the $default function', async () => {
    ...
    expect(XPathUtil.getPath(dataElements[0].getDefaultContext(), 'default')).to.equal(
      '$default/data[1]',
    );
    expect(XPathUtil.getPath(dataElements[1].getDefaultContext(), 'second')).to.equal(
      '$second/data[1]',
    );
    ...
    const el = await fixtureSync(html`
      <fx-fore>
        <fx-model id="model1">
          ...
          <fx-bind ref="$second/item" required="boolean-from-string('maybe!~')"></fx-bind>
        </fx-model>
      </fx-fore>
    `);
  ...
  it('uses correct content-type for xml', async () => {
    ...
    const dataElements = el.querySelectorAll('data');
    expect(Fore.getContentType(dataElements[0])).to.equal('application/xml; charset=UTF-8');
  });

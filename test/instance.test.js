...
describe('instance Tests', () => {
    ...
    it('evaluates xpath in its default context', async () => {
        ...
        expect(XPathUtil.getPath(result, 'default')).to.equal(
            '$default/foobar[1]',
        );
    });
    ...
    it('provides default evaluation context', async () => {
        ...
        expect(XPathUtil.getPath(context, 'default')).to.equal(
            '$default/data[1]',
        );
    });
    ...
    it('does NOT copy a "body" element from inline data', async () => {
        ...
        expect(XPathUtil.getPath(n, 'default')).to.equal(
            '$default/arm[1]/hand[1]/finger[1]',
        );
    ...
    it('resolves instances with the instance() function', async () => {
        ...
        expect(
            XPathUtil.getPath(instances[0].getDefaultContext(), 'default'),
        ).to.equal('$default/data[1]');
        expect(
            XPathUtil.getPath(instances[1].getDefaultContext(), 'second'),
        ).to.equal('$second/data[1]');
    ...
    it('uses correct content-type for xml', async () => {
        ...
        const instances = el.querySelectorAll('fx-instance');
        expect(Fore.getContentType(instances[0])).to.equal(
    ...
    it('uses correct content-type for json', async () => {
        const el = await fixtureSync(html`
            <fx-fore>
                <fx-model id="model1">
                    <fx-instance
                        src="base/test/automobiles.json"
                        type="json"
                    ></fx-instance>
                </fx-model>
            </fx-fore>
    ...
    it('loads inline json data', async () => {
            ...
            <fx-fore>
                <fx-model id="model1">
                    <fx-instance type="json">
                        { "automobiles": [ { "maker": "Nissan", "model":
                        "Teana", "year": 2000 }, { "maker": "Honda", "model":
                        "Jazz", "year": 2023 }, { "maker": "Honda", "model":
                        "Civic", "year": 2007 }, { "maker": "Toyota", "model":
                        "Yaris", "year": 2008 }, { "maker": "Honda", "model":
                        "Accord", "year": 2011 } ], "motorcycles": [{ "maker":
                        "Honda", "model": "ST1300", "year": 2012 }] }
                    </fx-instance>
                </fx-model>
            </fx-fore>
        `);
        ...
        expect(instances[0].instanceData.automobiles[0].maker).to.equal(
            'Nissan',
        );
    });

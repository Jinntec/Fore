...
describe('fx-items tests', () => {
        ...
        const el = await fixtureSync(html`
            <fx-fore>
                <fx-model id="model-1">
                    ...
                    <fx-instance
                        id="i-functions"
                        src="/base/test/functions.xml"
                    ></fx-instance>
                </fx-model>
                <fx-control ref="//m/@function" update-event="input">
                    <fx-items
                        ref="instance('i-functions')//option"
                        class="widget"
                    >
                        <template>
                            <span class="fx-checkbox">
    ...
    it('works when checkbox label is clicked', async () => {
        ...
        const checkboxes = el.querySelectorAll('input');
        expect(checkboxes.length).to.equal(11);
        expect(checkboxes[0].checked, 'Checkbox #0 should be checked or not').to
            .be.false;
        expect(checkboxes[1].checked, 'Checkbox #1 should be checked or not').to
            .be.true;
        expect(checkboxes[2].checked, 'Checkbox #2 should be checked or not').to
            .be.true;
        expect(checkboxes[3].checked, 'Checkbox #3 should be checked or not').to
            .be.true;
        expect(checkboxes[4].checked, 'Checkbox #4 should be checked or not').to
            .be.true;
        expect(checkboxes[5].checked, 'Checkbox #5 should be checked or not').to
            .be.false;
        expect(checkboxes[6].checked, 'Checkbox #6 should be checked or not').to
            .be.true;
        expect(checkboxes[7].checked, 'Checkbox #7 should be checked or not').to
            .be.false;
        expect(checkboxes[8].checked, 'Checkbox #8 should be checked or not').to
            .be.false;
        expect(checkboxes[9].checked, 'Checkbox #9 should be checked or not').to
            .be.false;
        expect(checkboxes[10].checked, 'Checkbox #10 should be checked or not')
            .to.be.false;

        const control = el.querySelector('fx-control');
        expect(control.value).to.equal('VAdj Part AgtNoun ActNoun PropN');
        expect(control.modelItem.value).to.equal(
            'VAdj Part AgtNoun ActNoun PropN',
        );
    });
    ...
    it('works with JSON data', async () => {
            ...
            <fx-fore>
                <fx-model id="model-1">
                    ...
                    <fx-instance id="list" type="json">
                        [ { "name": "Akklamation", "value":
                        "https://www.eagle-network.eu/voc/typeins/lod/73" }, {
                        "name": "Adnuntiatio", "value":
                        "https://www.eagle-network.eu/voc/typeins/lod/113" }, {
        ...
        expect(checkboxes[0].value).to.equal(
            'https://www.eagle-network.eu/voc/typeins/lod/73',
        );
        expect(checkboxes[1].value).to.equal(
            'https://www.eagle-network.eu/voc/typeins/lod/113',
        );
        expect(checkboxes[2].value).to.equal(
            'https://www.eagle-network.eu/voc/typeins/lod/116',
        );
            ...
            <fx-fore>
                <fx-model id="model-1">
                    <fx-instance>
                        <data>
                            <selected
                                >https://www.eagle-network.eu/voc/typeins/lod/73 https://www.eagle-network.eu/voc/typeins/lod/113</selected
                            >
                        </data>
                    </fx-instance>
                    <fx-instance id="list" type="json">

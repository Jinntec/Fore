...
describe('delete Tests', () => {
    it('deletes an item and sets index', async () => {
        const el = await fixtureSync(html`
            <fx-fore>
                <fx-model id="record">
                    <fx-instance>
                        <data>
                            <task complete="false" due="2019-02-04"
                                >Pick up Milk</task
                            >
                            <task complete="true" due="2019-01-04"
                                >Make tutorial part 1</task
                            >
                            <task complete="false" due="2019-01-05"
                                >third task</task
                            >
                            <task complete="false" due="2019-01-06"
                                >fourth task</task
                            >
                        </data>
                    </fx-instance>
        ...
        expect(rItems.length).to.equal(3, 'There should be three repeat items');
        //        expect(rItems[2].hasAttribute('repeat-index')).to.be.true;
        expect(repeat.getAttribute('index')).to.equal(
            '3',
            'The index of the repeat should be 3: the last index of the repeat',
        );
    });
    ...
    it('does not delete readonly item', async () => {
        ...
        const rItems1 = repeat.querySelectorAll('fx-repeatitem');
        expect(rItems1.length).to.equal(2, 'The item should have been removed');

        await button.performActions();

        const rItems2 = repeat.querySelectorAll('fx-repeatitem');
        expect(rItems2.length).to.equal(
            2,
            'The item should not have been removed',
        );
        expect(rItems2[0].hasAttribute('repeat-index')).to.be.true;
        //        expect(el.getModel().modelItems.length).to.equal(6);
    });
    ...
    it('deletes a set of nodes', async () => {
        ...
        const rItems = repeat.querySelectorAll('fx-repeatitem');
        expect(rItems.length).to.equal(
            0,
            'The repeat should have been emptied out',
        );

...
describe('template expressions', () => {
    it('detects template expressions', async () => {
        ...
        expect(theDiv.textContent).to.equal(
            'Greeting: Hello Universe another Hello Universe',
        );
    });

    it('detects template expressions in fx-groups', async () => {
        const el = await fixtureSync(html`
            <fx-fore>
                <fx-model>
                    <fx-instance>
                ...
                <fx-group>Greeting: {greeting} another {greeting}</fx-group>
            </fx-fore>
        `);

        await oneEvent(el, 'ready');
        const theGroup = el.querySelector('fx-group');

        expect(theGroup.textContent).to.equal(
            'Greeting: Hello Universe another Hello Universe',
        );
    });
...
greeting
        ...
        expect(div.textContent).to.equal(`Greeting: Hello Universe\n With new\nlines\n  another\n                    Hello Universe`);
    });
    ...
    it('Correctly resolves namespaces based on the context of the template', async () => {
        ...
        expect(theDivA.innerText).to.equal(
            'Greeting: Hello AAA another Hello AAA',
        );

        const theDivB = el.querySelector('.greetingB');
        expect(theDivB.getAttribute('class')).to.equal('greetingB Hello BBB');
        expect(theDivB.innerText).to.equal(
            'Greeting: Hello BBB another Hello BBB',
        );

        const theDivC = el.querySelector('.greetingC');
        expect(theDivC.getAttribute('class')).to.equal('greetingC Hello CCC');
        expect(theDivC.innerText).to.equal(
            'Greeting: Hello CCC another Hello CCC',
        );
    });
    ...
    it('does ignore elements matched by ignore-expression attribute', async () => {
        const el = await fixtureSync(html`
            <fx-fore ignore-expressions=".myElement">
                ...
                <div class="myElement">{ignored}{{whatever}}</div>
        ...
        expect(myElements[0].textContent.trim()).to.equal(
            '{ignored}{{whatever}}',
        );
        ...
        const el = await fixtureSync(html`
            <fx-fore ignore-expressions="pb-authority">
                ...
                <pb-authority
                    connector="Airtable"
                    name="person"
                    api-key="keyFpBEqgkRWCNrfK"
                    base="appcVM9MIZSxyvkCU"
                    table="People"
                    fields="Name, Direct Order Name, Variants"
                    label="{Name}"
                    tokenize="Name, Variants"
                    tokenize-regex="\\s*;\\s*"
                    filter="or(search('{key}', lower({Name})), search('{key}', lower({Direct Order Name})), search('{key}', lower({Variants})))"
                >
                    <template class="info">
                        <h3>{Name}</h3>
                        <p>{Variants}</p>
                    </template>
                    <template class="detail"> {Variants} </template>
                </pb-authority>
            </fx-fore>
        `);

    ...
    const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-model>
                    ...
                    <fx-bind ref="c" calculate="number(../a) * number(../b)" constraint="number(.) <= 100"
                             readonly="true()"></fx-bind>
                    <fx-bind ref="d" calculate="number(../a) + number(../b)" constraint="number(.) <= 20"
                             readonly="true()"></fx-bind>
                    <fx-bind ref="e" calculate="number(../a) + 5" constraint="number(.) <= 10"
                             readonly="true()"></fx-bind>
                    <fx-bind ref="y" calculate="number(../x) + 5" readonly="true()"
                             constraint="number(.) <= 10"></fx-bind>
                </fx-model>
                <fx-control ref="a" update-event="input">
                    <label>a</label>
                    <input type="number"/>
  ...
  it('recalcuates only the changed "a" subgraph of modelItems', async () => {
    const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-setvalue event="refresh-done" ref="a"
                ">11</fx-setvalue>
  ...
  it('recalcuates only the changed "b" subgraph of modelItems', async () => {
    const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-setvalue event="refresh-done" ref="b"
                ">11</fx-setvalue>
  ...
  it('recalcuates only the changed "x" subgraph of modelItems', async () => {
    const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-setvalue event="refresh-done" ref="x"
                ">6</fx-setvalue>
    ...
    const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-group>
                    ...
                    <p></p>
                    transform:
                    <fx-output ref="transform"></fx-output>
                    <p></p>
                    <fx-control
                            id="transform"
                            ref="string"

  ...
  it('context() in repeats returns the correct item', async () => {
    const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-repeat ref="$mapping/df[@tag = instance('desc')/df/tag]" id="outer-repeat">
                    <template>
                        ...
                        <fx-repeat ref="sf[@code = instance('desc')/df/sfs/sf/code]" id="inner-repeat">
                            <template>
                                ...
                                <p id="result-p-with-context" style="display: inline;">{$desc/df/sfs/sf[code
                                    = context()/@code]/value}</p>
                                <p id="result-p-with-current" style="display: inline;">{$desc/df/sfs/sf[code

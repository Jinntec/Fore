...
describe('insert Tests', () => {
    ...
    it('inserts at end by default', async () => {
        ...
        expect(tasks[2].getAttribute('complete')).to.equal(
            tasks[3].getAttribute('complete'),
        );
        expect(tasks[2].getAttribute('due')).to.equal(
            tasks[3].getAttribute('due'),
        );
    });
    ...
    it('inserts as first with position=before', async () => {
        ...
        expect(tasks[0].getAttribute('complete')).to.equal(
            tasks[3].getAttribute('complete'),
        );
        expect(tasks[0].getAttribute('due')).to.equal(
            tasks[3].getAttribute('due'),
        );
    });
    ...
    it('inserts after first with position=after', async () => {
        ...
        expect(tasks[1].getAttribute('complete')).to.equal(
            tasks[3].getAttribute('complete'),
        );
        expect(tasks[1].getAttribute('due')).to.equal(
            tasks[3].getAttribute('due'),
        );
    });
        ...
        const el = await fixtureSync(html`
            <fx-fore>
                ...
                <fx-trigger>
                    <button>insert at end</button>
                    <fx-insert
                        ref="instance('default')/data"
                        origin="instance('templ')/foo"
                    ></fx-insert>
                </fx-trigger>
            </fx-fore>
        `);
    ...
    it('inserts into inner repeat with "context" and "origin" attribute', async () => {
        ...
        const inst = el.getModel().getDefaultContext();
        console.log('instance after insert', inst);
        let item = fx.evaluateXPathToBoolean(
            "exists(//gramGrp[@type='segmentation'])",
            inst,
            null,
            {},
        ...
        expect(item).to.be.true;
        item = fx.evaluateXPathToBoolean(
            'exists(//listBibl/cit)',
            inst,
            null,
            {},

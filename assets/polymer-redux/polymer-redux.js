/// BareSpecifier=polymer-redux/polymer-redux
...
function bindToReduxStore(store, element, registry) {
	...
	const updateProperties = () => {
		if (typeof Definition.mapStateToProps === 'function') {
			const properties = Definition.mapStateToProps(store.getState(), element);
...
function addEventListeners(store, element) {
	...
	const listeners = typeof Definition.mapDispatchToEvents === 'function' ? Definition.mapDispatchToEvents(store.dispatch, element) : null;
	...
	return () => events.forEach(event => element.removeEventListener(event.event, event.handler));
}
...
function addEffectListeners(store, element, update) {
	...
	const notifications = notifyEffects ? Object.keys(notifyEffects).map(name => `${camelToDashCase(name)}-changed`) : [];

	notifications.forEach(name => element.addEventListener(name, update));

	return () => notifications.forEach(name => element.removeEventListener(name, update));
}
...
export function bindActionCreators(actionCreators, dispatch) {
	if (!actionCreators) {
		throw new TypeError('Polymer Redux: Expecting "actionCreators" to be an enumerable.');
	}

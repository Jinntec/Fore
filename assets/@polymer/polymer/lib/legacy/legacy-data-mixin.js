/// BareSpecifier=@polymer/polymer/lib/legacy/legacy-data-mixin
/**
@license
...
export const LegacyDataMixin = dedupingMixin(superClass => {
  ...
  class LegacyDataMixin extends superClass {
    ...
    _addPropertyEffect(property, type, effect) {
      return super._addPropertyEffect(property, type, wrapEffect(effect, effect && effect.info && effect.info.methodName));
    }
...
Polymer.Class = (info, mixin) => Class(info, superClass => mixin ? mixin(LegacyDataMixin(superClass)) : LegacyDataMixin(superClass));
 ...
 * @polymer
 */
const TemplatizeMixin = dedupingMixin(superClass => {
  /**
   * @constructor
   * @extends {HTMLElement}
   */
